...
upperKeys = fkMap +
  ...
  annotate(geom = "text", x = -80.34, y = 25.29, label = "C", size = 8) +
...
lowerKeys = fkMap +
  ...
  annotate(geom = "text", x = -81.69, y = 24.64, label = "B", size = 8) +
...
dryTortugas = fkMap +
  ...
  scale_y_continuous(breaks = c(seq(24.4, 24.7, by = .1))) +
  annotate(geom = "text", x = -83.24, y = 24.74, label = "A", size = 8) +
...
ggsave("../revisedFigures/fig1.pdf", plot=fknmsMap, height = 20, width = 30, unit = "cm", dpi = 300)
```
...
ggsave("../revisedFigures/suppFig2.tiff", plot = cloneDend, height = 8, width = 30, units = "in", dpi = 300)
```
...
heteroPlot <- ggplot(hetero, aes(x=popdepth, y=heterozygosity)) +
  geom_boxplot()+
  theme_bw()
heteroPlot
...
ggsave("../revisedFigures/fig2.pdf", plot = fkSnpPcoaPlot, height = 5, width = 7, units = "in", dpi = 300)
