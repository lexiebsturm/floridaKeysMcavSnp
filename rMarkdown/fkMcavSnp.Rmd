...
pacman::p_load("adegenet", "dendextend", "flextable", "gdata", "ggdendro", "hierfstat", "Imap", "kableExtra", "paletteer", "patchwork", "officer", "poppr", "RColorBrewer", "reshape2", "StAMPP", "tidyverse", "vcfR", "vegan", "WGCNA", "boa", "plyr", "rgdal", "broom", "rgeos", "ggmap", "moments", "car", "multcompView", "lsmeans", "ggsn", "sf","ggspatial")
```
...
```{r, permit map} 
 ...
 fknmsPermit <- readOGR("/Users/student/Documents/GitHub/floridaKeysMcavSnp/data/fknmsZones/FKNMS_Marine_Zones_1-polygon.shp") 
 fknmsshpfilePermit <- spTransform(fknmsPermit, "+init=epsg:4326") #WGS84 
 fknmsdtaPermit <- tidy(fknmsshpfilePermit, group=group) 
...
fknms <- readOGR("/Users/student/Documents/Florida\ Keys/Maps/fknms_py2/fknms_py.shp") 
 ...
 # plot map 
 #levels(fkSites$General.Site) 
 #fkSites$General.Site = factor(fkSites$General.Site, levels(fkSites$General.Site)[c(2,3,1,4)]) 
 
 fkMapPermit=ggmap(baseMap)+ 
   geom_polygon(aes(x=long, y=lat, group=group), 
                data=fknmsdtaPermit, 
                color="dark red", alpha=.2, size=.2) + 
     geom_polygon(aes(x=long, y=lat, group=group), 
                data=fknmsdta, 
                color="green", alpha=.2, size=.2) + 
   geom_point(data = fkSites, size= 4, aes(x = Lon, y = Lat, fill=General.Site, shape=Depth.Zone)) + 
...
#ggsave("../figures/fig1.eps", plot=fkMap, height = 20, width = 25, unit = "cm", dpi = 300)
```
...
ggsave("../figures/cloneDend.tiff", plot = cloneDend, height = 8, width = 30, units = "in", dpi = 300)
```
...
ggsave("../figures/fig2.eps", plot = fkSnpPcoaPlot, height = 5, width = 7, units = "in", dpi = 300, device=cairo_ps, fallback_resolution = 600)
...
ggsave("../figures/fig3.eps", plot = snpHeatmap, width = 34, height = 15, units = "cm", dpi = 300)
```
## ADMIXTURE Plot
```{r, ADMIXTURE}
...
fkK3$sample = factor(fkK3$sample, levels = fkK3$sample[order(-fkK3$cluster3)])
...
levels(mdat4$pop) = c("DRTS-Shallow", "DRTS-Mesophotic", "DRTN-Shallow", "DRTN-Mesophotic", "LK-Shallow", "LK-Mesophotic", "UK-Shallow", "UK-Mesophotic")
...
ggsave("../figures/combinedAdmix.tiff", plot = combinedAdmix, width = 60, height = 45, units = "cm", dpi = 300)

ggsave("../figures/admixturePlot.tiff", plot = p4, width = 30, height = 15, units = "cm", dpi = 300)

ggsave("../figures/fig4.eps", plot = combinedAdmix, width = 60, height = 45, units = "cm", dpi = 300)
```
