...
library(moments)
zooxHostProportions=read.csv("zooxHostProportions.csv", header=TRUE)
skewness(zooxHostProportions$fracZ, na.rm = TRUE)

zooxDepthPlot=ggplot(zooxHostProportions, aes(x=depth, y=fracZ)) +
  geom_point()+
...
leveneTest(fracZ ~ site*depthZone, data = zooxHostProportions)
res.aov3 <- aov(fracZ ~ site * depthZone, data = zooxHostProportions)
shapiro.test(zooxHostProportions$fracZ)
residualplot=plot(res.aov3, 2)
zooxAnova <- aov(fracZ ~ site * depthZone, data = zooxHostProportions)
Anova(zooxAnova, type = "III")

zooxBoxPlot=ggplot(zooxHostProportions, aes(x=site, y=fracZ, fill=depthZone)) +
  geom_boxplot()
...
zooxPlotA = ggplot(data = zDat, aes(x = Order, y = Fraction, fill = Symbiont, order = Order)) +
  geom_bar(stat = "identity", position = "fill", colour = "grey25", width = 1) +
  xlab("Population") +
  scale_x_discrete(expand = c(0, 0)) +
